<template>
  <div :style="{ width: width }" :class="multiple ? 'treeSelect_div_multiple' : 'treeSelect_div_radio'" class="treeSelect_div">
    <!-- `treeSelect_div${uuid}` -->
    <el-popover v-model="isShowSelect" :popper-class="'treeSelect_popover '+poperClass" placement="bottom-start" trigger="manual">
      <a-input-search v-if="remoteSearch && filterText" v-model="filterText" class="min_width150" :placeholder="filterPlaceholder" :style="{ width: selectWidth }" @search="onSearch">
        <a-button slot="enterButton" class="searchBtn" type="primary" icon="search" style="height: 34px">
          {{ remoteSearchText }}
        </a-button>
      </a-input-search>
      <div v-else>
        <el-checkbox v-if="multiple && showAllSelection" v-model="allSelection" :disabled="!data || data.length == 0" style="margin: 0 10px;" @change="allSelectionFun">全选</el-checkbox>
        <a-input v-model="filterText" class="min_width150" :style="{
            width:
              multiple && showAllSelection
                ? `calc(${selectWidth} - 90px)`
                : selectWidth
          }" :placeholder="filterPlaceholder"></a-input>
      </div>
      <div :style="style" :class="{
        ...
        <el-tree v-if="
            suggestKeys &&
              ...
              !filterText
          " ref="suggest_tree" class="suggest_tree" :data="suggestTree" :props="suggestProps" :show-checkbox="multiple" :node-key="nodeKey" :check-strictly="true" :default-expand-all="true" :expand-on-click-node="expandOnClickNode" :check-on-click-node="multiple" :highlight-current="false" @node-click="handleNodeClick1" @check-change="handleCheckChange1"></el-tree>
        <el-tree ref="treeSelect_tree" class="treeSelect_tree" :data="data" :props="defaultProps" :show-checkbox="multiple" :node-key="nodeKey" :check-strictly="checkStrictly" :default-expand-all="defaultExpandAll" :default-expanded-keys="defaultExpandedKeys" :expand-on-click-node="expandOnClickNode" :check-on-click-node="multiple" :filter-node-method="treeFilterNode" :render-content="renderContentFun" :highlight-current="true" @node-click="handleNodeClick" @check-change="handleCheckChange"></el-tree>
      </div>
      <el-select slot="reference" ref="treeSelect_select" v-model="selectValue" :placeholder="placeholder" :size="size" :disabled="readonly || disabled" :multiple="multiple" :clearable="clearable" :collapse-tags="collapseTags" class="treeSelect_select" :class="{
        ...
        }" @click.native="selectClick" @remove-tag="removeSelectedNodes" @clear="treeClear" @change="changeSelectedNodes">
